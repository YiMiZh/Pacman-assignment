...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            for i in range(0, self.numParticles):
                new.append(self.getJailPosition())
            self.particles = new
        else:
            ap = util.Counter()
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            list.append(util.sample(newPosDist))
        self.particles = list
        "*** END YOUR CODE HERE ***"
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        for par in self.particles:
            pro = 1
            for i in range(self.numGhosts):
                # Special case 2 Ignore captured gost
                if noisyDistances[i] == None:
                    par = self.getParticleWithGhostInJail(par, i)
                else:
                    d = util.manhattanDistance(par[i], pacmanPosition)
                    pro *= emissionModels[i][d]
            ap[par] += pro
        # Special case 1
        if ap.totalCount() == 0:
            self.initializeParticles()
